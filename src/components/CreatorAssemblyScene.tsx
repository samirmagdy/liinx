...
function AssemblyBlocks({ reducedMotion }: { reducedMotion: boolean }) {
  const group = useRef<Group>(null);
  const blocks = useMemo(() => [
    { position: [-1.35, 0.7, 0] as [number, number, number], size: [0.9, 0.18, 0.08] as [number, number, number], color: '#06B6D4' },
    { position: [0.9, 0.42, 0.08] as [number, number, number], size: [0.72, 0.3, 0.08] as [number, number, number], color: '#8B5CF6' },
    { position: [-0.65, -0.35, 0.12] as [number, number, number], size: [1.5, 0.12, 0.08] as [number, number, number], color: '#EC4899' },
    { position: [0.55, -0.84, 0] as [number, number, number], size: [0.95, 0.18, 0.08] as [number, number, number], color: '#6366F1' }
  ], []);
  ...
  return (
    <group ref={group} rotation={[0.08, -0.18, -0.04]}>
      ...
      {blocks.map((block) => (
        <mesh key={block.color} position={block.position}>
          <boxGeometry args={block.size} />
          <meshStandardMaterial color={block.color} emissive={block.color} emissiveIntensity={0.18} roughness={0.42} />
        </mesh>
      ))}
...
export function CreatorAssemblyScene() {
  ...
  return (
    <div ref={hostRef} className="creator-assembly-scene" aria-hidden="true">
      {!webglAvailable || reducedMotion ? (
        ...
        >
          ...
          <Suspense fallback={null}><AssemblyBlocks reducedMotion={reducedMotion} /></Suspense>
        </Canvas>
      ) : (
