...
type Guide = { path: string; title: string; summary: string; sections: { title: string; body: string }[] };

export function GuidesPage() {
  const { lang } = useLanguage();
  const ar = lang === 'ar';
  const [path, setLocation] = useLocation();
  const guides: Guide[] = ar ? [
    {
  ...
  return (
    <div className="min-h-screen bg-white text-neutral-900">
        ...
        {activeGuide ? (
          <article>
            ...
            <Reveal distance="md" className="mt-8 max-w-3xl">
              <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl text-balance">{activeGuide.title}</h1>
              <p className="mt-4 text-lg leading-relaxed text-neutral-600">{activeGuide.summary}</p>
            </Reveal>
            <div className="mt-10 divide-y divide-neutral-200 border-y border-neutral-200">
              {activeGuide.sections.map((section, index) => (
                <section key={section.title} className="grid gap-3 py-6 sm:grid-cols-[3rem_1fr]">
      ...
      <Footer onSelectView={view => { if (view === 'home') setLocation('/'); else if (view === 'builder') setLocation('/studio'); else if (view === 'templates') setLocation('/templates'); else if (view === 'pricing') setLocation('/pricing'); }} />
    </div>
  );
}
