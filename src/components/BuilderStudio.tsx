...
function StructuredItemsEditor({ kind, value, onChange, onUpload, ui }: { kind: StructuredEditorKind; value: any[]; onChange: (value: any[]) => void; onUpload?: (file: File, index: number, itemId?: string) => void; ui: (value: string) => string }) {
  const items = Array.isArray(value) ? value : [];
  const createItem = () => kind === 'form'
    ? { id: `field_${Date.now()}_${items.length}`, name: `field_${items.length + 1}`, label: 'New field', type: 'text', required: false }
    : kind === 'faq'
  ...
  return <div className="sm:col-span-2 space-y-2 rounded-xl border border-neutral-200 bg-neutral-50 p-3">
    <div className="flex items-center justify-between"><span className="text-[11px] font-bold text-neutral-700">{ui(kind === 'form' ? 'Form fields' : 'Items')}</span><button type="button" disabled={kind === 'form' && items.length >= 20} title={kind === 'form' && items.length >= 20 ? ui('Forms can contain at most 20 fields.') : undefined} onClick={() => onChange([...items, createItem()])} className="rounded-lg border border-neutral-300 px-2 py-1 text-[11px] font-semibold hover:border-neutral-900 disabled:cursor-not-allowed disabled:opacity-40">{ui('Add item')}</button></div>
    ...
    {items.map((item, index) => <div key={item.id || item.name || index} className="grid grid-cols-1 gap-2 rounded-lg border border-neutral-200 bg-white p-2 sm:grid-cols-[1fr_auto]">
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {kind === 'form' && <><input value={item.label || ''} onChange={e => update(index, 'label', e.target.value)} placeholder={ui('Label')} aria-label={ui('Field label')} maxLength={120} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><input value={item.name || ''} onChange={e => update(index, 'name', e.target.value.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64))} placeholder={ui('Field name')} aria-label={ui('Field name')} maxLength={64} className="rounded-lg border border-neutral-200 px-2 py-1.5 font-mono text-[11px] text-neutral-900" /><select value={item.type || 'text'} onChange={e => update(index, 'type', e.target.value)} aria-label={ui('Field type')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900"><option value="text">{ui('Text')}</option><option value="email">{ui('Email')}</option><option value="tel">{ui('Phone')}</option><option value="textarea">{ui('Long text')}</option></select><label className="flex items-center gap-2 text-[11px] text-neutral-700"><input type="checkbox" checked={item.required !== false} onChange={e => update(index, 'required', e.target.checked)} />{ui('Required')}</label><input value={item.helpText || ''} onChange={e => update(index, 'helpText', e.target.value)} placeholder={ui('Help text (optional)')} aria-label={ui('Field help text')} maxLength={300} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900 sm:col-span-2" /><label className="flex items-center gap-1 text-[11px] text-neutral-700">{ui('Minimum')}<input type="number" min="0" max="2000" value={item.minLength ?? ''} onChange={e => update(index, 'minLength', e.target.value === '' ? undefined : Number(e.target.value))} aria-label={ui('Minimum length')} className="w-20 rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /></label><label className="flex items-center gap-1 text-[11px] text-neutral-700">{ui('Maximum')}<input type="number" min="1" max="2000" value={item.maxLength ?? ''} onChange={e => update(index, 'maxLength', e.target.value === '' ? undefined : Number(e.target.value))} aria-label={ui('Maximum length')} className="w-20 rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /></label></>}
        {kind === 'gallery' || kind === 'carousel' ? <><input value={item.imageUrl || ''} onChange={e => update(index, 'imageUrl', e.target.value)} placeholder={ui('Image URL')} aria-label={ui('Image URL')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900 sm:col-span-2" /><input type="file" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file && onUpload) onUpload(file, index, item.id); }} aria-label={ui('Upload image')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><input value={item.alt || ''} onChange={e => update(index, 'alt', e.target.value)} placeholder={ui('Alt text')} aria-label={ui('Alt text')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><input value={item.caption || ''} onChange={e => update(index, 'caption', e.target.value)} placeholder={ui('Caption')} aria-label={ui('Caption')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><input value={item.linkUrl || ''} onChange={e => update(index, 'linkUrl', e.target.value)} placeholder={ui('Optional link URL')} aria-label={ui('Optional image link URL')} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900 sm:col-span-2" /></> : null}
        {kind === 'faq' && <><input value={item.question || ''} onChange={e => update(index, 'question', e.target.value)} placeholder={ui('Question')} aria-label={ui('Question')} maxLength={300} className="rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><textarea value={item.answer || ''} onChange={e => update(index, 'answer', e.target.value)} placeholder={ui('Answer')} aria-label={ui('Answer')} maxLength={5000} className="min-h-24 rounded-lg border border-neutral-200 px-2 py-1.5 text-[11px] text-neutral-900" /><p className="text-[10px] text-neutral-500 sm:col-span-2">{ui('Answers are plain text. Line breaks are preserved; links are not supported in FAQs.')}</p></>}
      ...
      <div className="flex items-start justify-end gap-1"><button type="button" onClick={() => move(index, -1)} disabled={index === 0} aria-label={ui('Move item up')} className="rounded p-1 text-neutral-500 disabled:opacity-30">↑</button><button type="button" onClick={() => move(index, 1)} disabled={index === items.length - 1} aria-label={ui('Move item down')} className="rounded p-1 text-neutral-500 disabled:opacity-30">↓</button>{kind === 'form' && <button type="button" onClick={() => { const base = String(item.name || 'field').slice(0, 55); let name = `${base}_copy`; let suffix = 2; while (items.some((candidate, candidateIndex) => candidateIndex !== index && candidate.name === name)) name = `${base}_copy_${suffix++}`; onChange([...items.slice(0, index + 1), { ...item, id: `field_${Date.now()}_${index}`, name }, ...items.slice(index + 1)]); }} disabled={items.length >= 20} aria-label={ui('Duplicate field')} className="rounded p-1 text-neutral-500 disabled:opacity-30">+</button>}<button type="button" onClick={() => remove(index)} aria-label={ui(kind === 'form' ? 'Delete field' : 'Remove item')} className="rounded p-1 text-rose-600">×</button></div>
    </div>)}
  </div>;
}
...
export const BuilderStudio: React.FC<BuilderStudioProps> = ({
  ...
  const syncProfileScopedState = (nextProfile: CreatorProfile) => {
    ...
    setFooterLogoLinkInput(nextProfile.footerLogoLink || '');
    setFooterLogoAltInput(nextProfile.footerLogoAlt || '');
  ...
  const [footerLogoLinkInput, setFooterLogoLinkInput] = useState(profile.footerLogoLink || '');
  const [footerLogoAltInput, setFooterLogoAltInput] = useState(profile.footerLogoAlt || '');
  ...
  useEffect(() => {
    ...
    setFooterLogoLinkInput(profile.footerLogoLink || '');
    setFooterLogoAltInput(profile.footerLogoAlt || '');
  ...
  const handleBlockImageUpload = async (blockId: string, file: File, field = 'imageUrl', itemIndex?: number, itemId?: string) => {
    try {
      ...
      if (itemId && !current.some((item: any) => item?.id === itemId)) throw new Error(ui('This gallery item was removed before the upload finished.'));
      const nextItems = itemIndex === undefined && !itemId ? current : current.map((item: any, index: number) => (itemId ? item?.id === itemId : index === itemIndex) ? { ...item, [field]: uploaded.url } : item);
                  ...
                  >
                    ...
                    {['rich_text', 'image', 'gallery', 'carousel', 'spacer', 'form', 'download', 'map', 'faq', 'testimonials', 'event', 'presave', 'phone', 'product', 'tips', 'content_gate'].includes(block.type) && (() => {
                      ...
                      return <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2 border-t border-neutral-100 text-xs">
                        {(['rich_text', 'form', 'download', 'map', 'content_gate'].includes(block.type)) && <div className="sm:col-span-2"><label className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui(block.type === 'rich_text' || block.type === 'content_gate' ? 'Content' : 'Description')}</label>{block.type === 'rich_text' && <><div className="mb-1 flex flex-wrap items-center gap-1"><button type="button" aria-label={ui('Bold selected text')} onClick={() => formatRichText('**')} className="rounded border border-neutral-200 px-2 py-1 text-[10px] font-bold">B</button><button type="button" aria-label={ui('Italic selected text')} onClick={() => formatRichText('*')} className="rounded border border-neutral-200 px-2 py-1 text-[10px] italic">I</button><button type="button" aria-label={ui('Heading line')} onClick={() => formatRichText('## ')} className="rounded border border-neutral-200 px-2 py-1 text-[10px] font-bold">H</button><button type="button" aria-label={ui('Bulleted list line')} onClick={() => formatRichText('- ')} className="rounded border border-neutral-200 px-2 py-1 text-[10px]">•</button><button type="button" aria-label={ui('Numbered list line')} onClick={() => formatRichText('1. ')} className="rounded border border-neutral-200 px-2 py-1 text-[10px]">1.</button></div><div className="mb-1 flex gap-1"><input aria-label={ui('Safe link URL')} value={richTextLinkUrl} onChange={e => setRichTextLinkUrl(e.target.value)} placeholder="https://..." className="min-w-0 flex-1 rounded border border-neutral-200 px-2 py-1 text-[10px] text-neutral-900" /><button type="button" aria-label={ui('Insert link')} disabled={!richTextLinkUrl.trim()} onClick={() => { const textarea = document.getElementById(`rich-text-${block.id}`) as HTMLTextAreaElement | null; if (!textarea) return; const current = String(advanced.body || ''); const start = textarea.selectionStart; const end = textarea.selectionEnd; const selected = current.slice(start, end); if (!selected) return; try { const parsed = new URL(richTextLinkUrl.trim()); if (!['http:', 'https:', 'mailto:', 'tel:'].includes(parsed.protocol)) return; const replacement = `[${selected}](${richTextLinkUrl.trim()})`; updateAdvanced('body', `${current.slice(0, start)}${replacement}${current.slice(end)}`); requestAnimationFrame(() => { textarea.focus(); textarea.setSelectionRange(start + replacement.length, start + replacement.length); }); } catch { return; } }} className="rounded border border-neutral-200 px-2 py-1 text-[10px] font-semibold disabled:opacity-40">{ui('Link')}</button></div><p className="mb-1 text-[10px] text-neutral-500">{ui('Safe format: paragraphs, # headings, - or 1. lists, **bold**, *italic*, and [label](https://url). HTML is shown as text.')}</p></>}<textarea id={block.type === 'rich_text' ? `rich-text-${block.id}` : undefined} aria-label={ui(block.type === 'rich_text' ? 'Rich text content' : 'Content')} value={advanced.body || advanced.description || ''} onChange={e => updateAdvanced(block.type === 'rich_text' || block.type === 'content_gate' ? 'body' : 'description', e.target.value)} className="w-full min-h-20 px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></div>}
                        {block.type === 'image' && <div className="space-y-2 sm:col-span-2"><label className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui('Image URL')}<input value={advanced.imageUrl || ''} onChange={e => updateAdvanced('imageUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><input type="file" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file) void handleBlockImageUpload(block.id, file); e.currentTarget.value = ''; }} aria-label={ui('Upload or replace image')} className="w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Alt text')}<input value={advanced.alt || ''} onChange={e => updateAdvanced('alt', e.target.value)} maxLength={300} placeholder={ui('Describe the image')} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="flex items-end gap-2 pb-1 text-[11px] font-semibold text-neutral-500"><input type="checkbox" checked={Boolean(advanced.decorative)} onChange={e => updateAdvanced('decorative', e.target.checked)} />{ui('Decorative image')}</label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Caption')}<input value={advanced.caption || ''} onChange={e => updateAdvanced('caption', e.target.value)} maxLength={500} placeholder={ui('Optional caption')} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Destination URL')}<input value={advanced.linkUrl || ''} onChange={e => updateAdvanced('linkUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Image fit')}<select value={advanced.fit || 'cover'} onChange={e => updateAdvanced('fit', e.target.value)} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900"><option value="cover">{ui('Crop to fill')}</option><option value="contain">{ui('Show full image')}</option></select></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Image shape')}<select value={advanced.aspect || 'auto'} onChange={e => updateAdvanced('aspect', e.target.value)} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900"><option value="auto">{ui('Natural')}</option><option value="square">{ui('Square')}</option><option value="portrait">{ui('Portrait')}</option><option value="landscape">{ui('Landscape')}</option></select></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Crop position')}<select value={advanced.cropPosition || 'center'} onChange={e => updateAdvanced('cropPosition', e.target.value)} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900"><option value="center">{ui('Center')}</option><option value="top">{ui('Top')}</option><option value="bottom">{ui('Bottom')}</option><option value="left">{ui('Left')}</option><option value="right">{ui('Right')}</option></select></label></div><p className="text-[10px] text-neutral-500">{ui('Informative images need alt text. Mark an image decorative when it adds no meaning. Uploading a replacement does not delete the previous asset.')}</p></div>}
                        {['download', 'event', 'presave', 'product', 'tips'].includes(block.type) && <div><label className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui('Destination URL')}</label><input value={advanced.fileUrl || advanced.url || ''} onChange={e => updateAdvanced(block.type === 'download' ? 'fileUrl' : 'url', e.target.value)} placeholder="https://..." className="w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" />{block.type === 'download' && <input type="file" onChange={e => { const file = e.target.files?.[0]; if (file) void handleBlockFileUpload(block.id, file); }} aria-label={ui('Upload downloadable file')} className="mt-2 w-full rounded-lg border border-neutral-200 bg-neutral-50 px-2.5 py-1.5 text-[11px] text-neutral-900" />}</div>}
                        {block.type === 'event' && <div className="grid grid-cols-1 gap-2 sm:col-span-2 sm:grid-cols-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Event description')}<textarea value={advanced.description || ''} onChange={e => updateAdvanced('description', e.target.value)} maxLength={1000} className="mt-1 min-h-20 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Artwork URL')}<input value={advanced.artworkUrl || ''} onChange={e => updateAdvanced('artworkUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Date')}<input value={advanced.date || ''} onChange={e => updateAdvanced('date', e.target.value)} maxLength={100} placeholder={ui('e.g. 12 October 2026')} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Time')}<input value={advanced.time || ''} onChange={e => updateAdvanced('time', e.target.value)} maxLength={50} placeholder={ui('e.g. 7:00 PM')} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Timezone')}<input value={advanced.timezone || ''} onChange={e => updateAdvanced('timezone', e.target.value)} maxLength={80} placeholder={ui('e.g. AST or UTC+3')} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Location')}<input value={advanced.location || ''} onChange={e => updateAdvanced('location', e.target.value)} maxLength={300} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><p className="text-[10px] font-normal text-neutral-500 sm:col-span-2">{ui('Date and time are displayed exactly as entered. Include the event timezone; Liinx does not convert times for visitors.')}</p></div>}
                        {block.type === 'presave' && <div className="sm:col-span-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Release description')}<textarea value={advanced.description || ''} onChange={e => updateAdvanced('description', e.target.value)} maxLength={1000} className="mt-1 min-h-16 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><p className="mt-1 text-[10px] font-normal text-neutral-500">{ui('External release link only. Liinx does not complete a music-service pre-save or request authorization.')}</p></div>}
                        {block.type === 'product' && <div className="grid grid-cols-1 gap-2 sm:col-span-2 sm:grid-cols-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Product description')}<textarea value={advanced.description || ''} onChange={e => updateAdvanced('description', e.target.value)} maxLength={1000} className="mt-1 min-h-20 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Product image URL')}<input value={advanced.imageUrl || ''} onChange={e => updateAdvanced('imageUrl', e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Price amount')}<input value={advanced.priceAmount || ''} onChange={e => updateAdvanced('priceAmount', e.target.value)} inputMode="decimal" maxLength={11} placeholder={ui('e.g. 29.00')} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Currency code')}<input value={advanced.currency || ''} onChange={e => updateAdvanced('currency', e.target.value.toUpperCase().replace(/[^A-Z]/g, '').slice(0, 3))} maxLength={3} placeholder={ui('e.g. USD')} className="mt-1 w-full uppercase rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><p className="text-[10px] font-normal text-neutral-500 sm:col-span-2">{ui('Display-only price. Liinx does not process checkout, inventory, taxes, or payments.')}</p></div>}
                        {block.type === 'tips' && <div className="sm:col-span-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Support description')}<textarea value={advanced.description || ''} onChange={e => updateAdvanced('description', e.target.value)} maxLength={1000} className="mt-1 min-h-16 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><p className="mt-1 text-[10px] font-normal text-neutral-500">{ui('External support link only. Liinx does not process tips or show earnings.')}</p></div>}
                        {block.type === 'map' && <div><label className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui('Address or place to find')}<input value={advanced.location || ''} onChange={e => updateAdvanced('location', e.target.value)} maxLength={300} placeholder={ui('City or address')} className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><p className="text-[10px] font-normal text-neutral-500">{ui('Directions link only. No embedded map or device location request is used.')}</p></div>}
                        {block.type === 'phone' && <div className="grid grid-cols-1 gap-2 sm:col-span-2 sm:grid-cols-2"><label className="block text-[11px] font-semibold text-neutral-500">{ui('Contact action')}<select value={advanced.contactType || 'phone'} onChange={e => updateAdvanced('contactType', e.target.value)} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900"><option value="phone">{ui('Click to call')}</option><option value="email">{ui('Send email')}</option></select></label>{advanced.contactType === 'email' ? <label className="block text-[11px] font-semibold text-neutral-500">{ui('Email address')}<input value={advanced.email || ''} onChange={e => updateAdvanced('email', e.target.value)} type="email" dir="ltr" placeholder="hello@example.com" className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label> : <label className="block text-[11px] font-semibold text-neutral-500">{ui('Phone number')}<input value={advanced.phone || ''} onChange={e => updateAdvanced('phone', e.target.value)} dir="ltr" inputMode="tel" placeholder="+966 50 123 4567" className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /><span className="mt-1 block text-[10px] font-normal text-neutral-500">{ui('Include country code; spaces, parentheses, and hyphens are allowed.')}</span></label>}<label className="block text-[11px] font-semibold text-neutral-500">{ui('Availability')}<input value={advanced.availability || ''} onChange={e => updateAdvanced('availability', e.target.value)} maxLength={200} placeholder={ui('Optional availability')} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Contact description')}<textarea value={advanced.description || ''} onChange={e => updateAdvanced('description', e.target.value)} maxLength={1000} className="mt-1 min-h-16 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label>{advanced.contactType === 'email' && <><label className="block text-[11px] font-semibold text-neutral-500">{ui('Email subject')}<input value={advanced.subject || ''} onChange={e => updateAdvanced('subject', e.target.value)} maxLength={200} className="mt-1 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label><label className="block text-[11px] font-semibold text-neutral-500">{ui('Email message')}<textarea value={advanced.body || ''} onChange={e => updateAdvanced('body', e.target.value)} maxLength={1000} className="mt-1 min-h-16 w-full rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /></label></>}<p className="text-[10px] font-normal text-neutral-500 sm:col-span-2">{ui('This is a creator-published contact action, not a form. Visitors are not asked to submit data to Liinx.')}</p></div>}
                        {block.type === 'form' && <div className="sm:col-span-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-900"><label className="flex items-start gap-2 font-semibold"><input type="checkbox" checked={advanced.consentRequired === true} onChange={e => updateAdvanced('consentRequired', e.target.checked)} className="mt-0.5" />{ui('Require visitor consent before storing responses')}</label><p className="mt-1 pl-5 font-normal">{ui('Do not imply consent is collected unless this is enabled. Liinx does not send notifications automatically.')}</p>{advanced.consentRequired === true && <input value={advanced.consentText || ''} onChange={e => updateAdvanced('consentText', e.target.value)} maxLength={300} placeholder={ui('Consent text (optional)')} aria-label={ui('Consent text')} className="mt-2 w-full rounded-lg border border-amber-200 bg-white px-2 py-1.5 text-[11px] text-neutral-900" />}</div>}
                        {['gallery', 'carousel', 'faq', 'testimonials', 'form'].includes(block.type) && <StructuredItemsEditor kind={block.type as StructuredEditorKind} value={advanced.items || advanced.fields || []} onChange={items => updateAdvanced(block.type === 'form' ? 'fields' : 'items', items)} onUpload={(file, _index, itemId) => void handleBlockImageUpload(block.id, file, 'imageUrl', undefined, itemId)} ui={ui} />}
                        {block.type === 'spacer' && (() => {
                          ...
                          return <div><label htmlFor={`spacer-height-${block.id}`} className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui('Height (px)')}</label><input id={`spacer-height-${block.id}`} type="number" min="16" max="240" step="1" value={spacerHeight} onChange={e => { const next = Number(e.target.value); updateAdvanced('height', Number.isFinite(next) ? Math.min(240, Math.max(16, next)) : 48); }} aria-describedby={`spacer-help-${block.id}`} className="w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /><p id={`spacer-help-${block.id}`} className="mt-1 text-[10px] font-normal text-neutral-500">{ui(`Adds ${spacerHeight}px of intentional space. Normal block gaps are not added around this spacer.`)}</p></div>;
                        })()}
                        {block.type === 'content_gate' && <div><label className="block text-[11px] font-semibold text-neutral-500 mb-1">{ui('Access code')}</label><input type="password" value={advanced.password || ''} onChange={e => updateAdvanced('password', e.target.value)} placeholder={advanced.locked ? ui('Leave blank to keep the current code; enter a new code to replace it.') : ui('Set a code to protect this text.')} maxLength={128} className="w-full px-2.5 py-1.5 rounded-lg border border-neutral-200 bg-neutral-50 text-neutral-900" /><p className="mt-1 text-[10px] font-normal text-neutral-500">{ui('Content gates protect inline text only. They do not provide membership or identity verification.')}</p></div>}
                      </div>;
                    })()}
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-neutral-50 p-5 rounded-2xl border border-neutral-200 shadow-xs space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <label className="text-xs font-semibold text-neutral-800">{ui('Footer logo destination')}<input type="url" disabled={profile.plan === 'free'} value={footerLogoLinkInput} onChange={e => setFooterLogoLinkInput(e.target.value)} className="mt-1 w-full rounded-xl border border-neutral-200 bg-neutral-50 p-2.5 text-xs font-normal text-neutral-900 disabled:opacity-50" placeholder="https://..." /></label>
                  <label className="sm:col-span-2 text-xs font-semibold text-neutral-800">{ui('Footer logo accessible name')}<input type="text" disabled={profile.plan === 'free'} value={footerLogoAltInput} onChange={e => setFooterLogoAltInput(e.target.value)} maxLength={120} className="mt-1 w-full rounded-xl border border-neutral-200 bg-neutral-50 p-2.5 text-xs font-normal text-neutral-900 disabled:opacity-50" placeholder={profile.displayName} /></label>
