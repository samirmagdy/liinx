...
const AdvancedPublicBlock: React.FC<{ block: any; profileId: string; theme: ThemeConfig; previewOnly?: boolean; translate?: (value: string) => string; blockIndex?: number; blockCount?: number }> = ({ block, profileId, theme, previewOnly = false, translate = value => value, blockIndex = 0, blockCount = 1 }) => {
  ...
  if (block.type === 'content_gate') return <div className={card} style={cardStyle}>{unlocked ? <div className="whitespace-pre-wrap text-sm" dir="auto">{status}</div> : !extra.locked ? <><h3 className="font-bold" dir="auto">{block.title}</h3><p role="status" className="text-sm my-3" style={{ color: theme.subtextColor }}>{translate('This protected text is not configured yet.')}</p></> : <form onSubmit={async e => { e.preventDefault(); setStatus(translate('Checking…')); try { const response = await fetch('/api/content-gates/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profileId, blockId: block.id, password: gateValue }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || translate('Unable to unlock')); setStatus(data.body || translate('Unlocked.')); setUnlocked(true); } catch (error) { setStatus(error instanceof Error ? error.message : translate('Unable to unlock.')); } }}><h3 className="font-bold" dir="auto">{block.title}</h3><p className="text-sm my-3" style={{ color: theme.subtextColor }} dir="auto">{extra.description || translate('Enter the access code to continue.')}</p><input required type="password" value={gateValue} onChange={e => setGateValue(e.target.value)} className="w-full rounded-xl border bg-transparent p-3 mb-2" placeholder={translate('Access code')} dir="ltr" /><button className="rounded-xl px-4 py-2 text-sm font-bold" style={{ backgroundColor: theme.accentColor, color: getAccessibleTextColor(theme.accentColor) }}>{translate('Unlock')}</button>{status && <p role="alert" className="text-xs mt-2" dir="auto">{status}</p>}</form>}</div>;
  return null;
};
