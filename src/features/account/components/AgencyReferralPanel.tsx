...
export function AgencyReferralPanel({ ar }: { ar: boolean }) {
  ...
  return (
    <section className="space-y-5" aria-labelledby="agency-referrals-title">
      <div>
        <div className="flex items-center gap-2 text-amber-700"><BadgeDollarSign className="h-4 w-4" /><span className="text-xs font-semibold uppercase tracking-caps">{ar ? 'برنامج إحالة الوكالات' : 'Agency referral program'}</span></div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-neutral-200 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-neutral-900">{data.creditedThisYear}<span className="text-base font-medium text-neutral-600"> / {data.cap}</span></p>
