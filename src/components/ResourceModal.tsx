...
export function ResourceModal({ isOpen, initialDoc, onClose }: ResourceModalProps) {
  ...
  const docs: Record<ResourceDocType, { title: string; body: string; href?: string }> = {
    ...
    'switch-linktree': {
      title: ar ? 'استيراد الروابط' : 'Import links',
      body: hasAnyImporter
        ? (ar ? 'افتح أداة الاستيراد في الاستوديو، أدخل رابط صفحتك العامة، راجع الروابط واختر ما تريد حفظه. قد تمنع بعض المواقع قراءة محتواها.' : 'Open the importer in Studio, enter your public page URL, review the extracted links and select which to save. Some websites may block extraction.')
        : (ar ? 'الاستيراد التلقائي متوقف حالياً لحين إتاحة تكاملات API الرسمية. يمكنك إضافة روابطك وتخصيصها مباشرة عبر الاستوديو.' : 'Automated imports are currently paused pending authorized provider APIs. You can easily add and customize your links directly in Studio.'),
      href: '/studio'
    },
