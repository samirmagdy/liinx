...
function InformationPage({ title, sections }: { title: string; sections: [string, string][] }) {
  const { tr: ui } = useUiLanguage();
  return <div className="min-h-screen flex flex-col bg-white"><Navbar />
...
export function PrivacyPage() {
  ...
  return <InformationPage title={lang === 'ar' ? 'الخصوصية' : 'Privacy'} sections={lang === 'ar' ? [
...
export function TermsPage() {
  const { lang } = useLanguage();
  return <InformationPage title={lang === 'ar' ? 'شروط الاستخدام' : 'Terms of use'} sections={lang === 'ar' ? [
    ...
    <div className="min-h-screen flex flex-col bg-white text-neutral-900">
      ...
      <main className="flex-1 max-w-3xl mx-auto px-6 py-16">
        <div className="text-center space-y-4 mb-12">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight">{ui("We'd love to hear from you.")}</h1>
